...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # (1)
        initial_hidden_states = np.add(np.matmul(features, W_proj), b_proj) # shape = (N, H)
        # (2)
        x, cache =  word_embedding_forward(captions_in,  W_embed) # shape = (N, T, W)
        # (3)
        hidden_states, cache_vrnn = rnn_forward(x, initial_hidden_states, Wx, Wh, b) # shape = (N, T, H)
        # (4)
        t_out, cache_taf = temporal_affine_forward( hidden_states, W_vocab, b_vocab)
        # (5)
        ...
        # (4)
        dt_out, dW_vocab, db_vocab = temporal_affine_backward(dout, cache_taf)
        # (3)
        dx, dh0, dWx, dWh, db =  rnn_backward(dt_out, cache_vrnn)
        # (2)
        dW_embed = word_embedding_backward(dx, cache)
        # (1)
        dW_proj = np.matmul(features.T, dh0)
        db_proj = np.sum(dh0, axis=0)
        
        # filling in grads dictionary 
        grads = {'Wx':dWx, 
                 'Wh':dWh, 
                 'b':db, 
                 'W_vocab':dW_vocab, 
    ...
    def clip_grad_norm(self, grads, gclip):
        ...
        for key in grads:
            ...
            if grad_norm > gclip:
                clipped_grads[key] = (grad * gclip) / grad_norm
            else:
    ...
    def sample_greedily(self, features, max_length=30):
        ...
        initial_state = np.add(np.matmul(features, W_proj), b_proj)
        
        x = np.zeros(shape=(N, W_embed.shape[1])) * W_embed[self._start]
        
        for i in range(max_length):
            next_h, cache = rnn_step_forward(x, initial_state, Wx, Wh, b) # shape = ()
            out = np.add(np.matmul(next_h, W_vocab), b_vocab) # shape = ()
            max_score = np.argmax(out, axis=1)
            captions[:,i] = max_score
            x = W_embed[max_score]
            initial_state = next_h
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample_randomly(self, features, max_length=30):
        ...
        for i in range(max_length):
            next_h, cache = rnn_step_forward(x, initial_state, Wx, Wh, b)
            out = np.add(np.matmul(next_h, W_vocab), b_vocab)
            softmax_probs = softmax(out)
            random_index = np.random.choice(out.shape[1], 1, p=softmax_probs)
            captions[:,i] =  random_index
            x = W_embed[random_index]
            initial_state = next_h

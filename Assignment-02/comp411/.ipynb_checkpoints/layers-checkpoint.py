...
def affine_backward(dout, cache):
    ...
    x_trans = np.reshape(x, (N, D)).T
    dw = np.matmul(x_trans, dout)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        N = x.shape[0]
        mask = (np.random.rand(N) < p) / p
        out = np.reshape(mask, (-1, 1)) * x
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for index in range(N):
        X = np.pad(x[index], ((0,0), (pad,pad), (pad, pad)))

        for filter in range(F):
            for i in range(0, H_out):
                for j in range(W_out):
                    ver_start = i * stride
                    ver_end = ver_start + HH
                    horiz_start = j * stride
                    horiz_end = horiz_start + WW
                    img_slice = X[:,  ver_start: ver_end, horiz_start:horiz_end]
                  
                    neuron = np.sum(img_slice * w[filter]) + b[filter]
                    out[index, filter, i, j] = neuron         
...
def max_pool_forward_naive(x, pool_param):
    ...
    N, C, H, W = x.shape
    H_out = 1 + int((H - p_H) / stride)
    W_out = 1 + int((W - p_W) / stride)
    ...
    for index in range(N):
        for i in range(H_out):
            for j in range(W_out):
                ver_start = i * stride
                ver_end = ver_start + p_H
                horiz_start = j * stride
                horiz_end = horiz_start + p_W
                img_slice = x[index, :,  ver_start: ver_end, horiz_start:horiz_end]
                neuron = np.amax(img_slice, axis=(-1,-2))
                out[index, :, i, j] = neuron   
...
def max_pool_backward_naive(dout, cache):
    ...
    x, pool_param = cache
    N, C, H, W = x.shape
    HH, WW = dout.shape[-2], dout.shape[-1]
        ...
        for filter in range(C):
            for i in range(HH):
                for j in range(WW):
                    ...
                    horiz_start = j * stride
                    horiz_end = horiz_start + p_W
                    img_slice = x[index, filter,  ver_start: ver_end, horiz_start:horiz_end]
                    mask = (np.max(img_slice) == img_slice)
                    dx[index, filter, ver_start:ver_end, horiz_start:horiz_end] = dout[index, filter, i, j] * mask

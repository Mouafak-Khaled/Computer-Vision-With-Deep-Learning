...
class ThreeLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=(64, 32), num_classes=10,
        ...
        self.params['W1'] = np.random.normal(0.0, weight_scale, (input_dim, hidden_dim[0]))
        self.params['b1'] = np.zeros(hidden_dim[0])
        self.params['W2'] = np.random.normal(0.0, weight_scale, (hidden_dim[0], hidden_dim[1]))
        self.params['b2'] = np.zeros(hidden_dim[1])
        self.params['W3'] = np.random.normal(0.0, weight_scale, (hidden_dim[1], num_classes))
    ...
    def loss(self, X, y=None):
        ...
        A1, cache = affine_lrelu_forward(X, W1, b1, lrelu_param)
        A2 , cache2 = affine_lrelu_forward(A1, W2, b2, lrelu_param)
        ...
        dA1, dW2, db2 = affine_lrelu_backward(dA2, cache2)
        dX, dW1, db1 = affine_lrelu_backward(dA1, cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        layers_dims = hidden_dims
        layers_dims.insert(0, input_dim)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            X, cache = affine_lrelu_forward(X, W, b, lrelu_param)
            caches['cache' + str(i + 1)] = cache
            if self.use_dropout:
                X, drop_cache = dropout_forward(X, self.dropout_param)
                caches['drop_cache' + str(i + 1)] = drop_cache

            
        scores, cache_l = affine_forward(X, self.params['W' + str(self.num_layers)],  self.params['b' + str(self.num_layers)])

    
        # dropout_backward(dout, cache)
        ...
        dout, dW, db = affine_backward(dout, cache_l)
        grads["W" + str(self.num_layers)] = dW  +  self.reg * self.params['W' + str(self.num_layers)]
        grads["b" + str(self.num_layers)] = db
        for i in range(self.num_layers, 1, -1):
            
            if self.use_dropout:
                dout = dropout_backward(dout, caches['drop_cache' + str(i - 1)])
            dout, dW, db = affine_lrelu_backward(dout, caches['cache' + str(i - 1)])

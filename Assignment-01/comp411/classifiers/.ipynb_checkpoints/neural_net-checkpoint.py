...
class ThreeLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        loss = np.sum(-np.log(softmax_term[np.arange(N), y]))
        softmax_term[np.arange(N),y] -= 1
        ...
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        dZ3 = softmax_term 
        grads["W3"] = (1 / N) * np.matmul(A2.T, dZ3)
        grads["b3"] = (1 / N) * np.sum(dZ3, axis=0)
        
        dZ2 = np.matmul(dZ3, W3.T) * (Z2 > 0)
        grads["W2"] = (1 / N) * np.matmul(A1.T, dZ2)
        grads["b2"] = (1 / N) * np.sum(dZ2, axis=0)
       
        dZ1 = np.matmul(dZ2, W2.T) * (Z1 > 0)
        grads["W1"] = (1 / N) * np.matmul(X.T, dZ1)

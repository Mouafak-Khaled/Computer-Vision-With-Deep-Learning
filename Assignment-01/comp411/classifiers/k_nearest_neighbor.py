...
class KNearestNeighbor(object):
    ...
    def compute_L2_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                L2 = np.sum((self.X_train[j] - X[i]) ** 2)
    ...
    def compute_L2_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            L2 = np.sum((self.X_train - X[i]) ** 2, axis=1)
            dists[i, :] = np.sqrt(L2)
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
    ...
    def compute_L2_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        A_square = self.X_train ** 2
        B_square = X ** 2
        dot_term = -2 * np.dot(X, self.X_train.T)
        L2 = np.matmul(A_square, np.ones(shape=(D, num_test))).T + np.matmul(B_square, np.ones(shape=(D, num_train))) + dot_term
        dists = np.sqrt(L2)
    ...
    def compute_L1_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        dists = self.X_train[:, None, :] - X[None, :, :]
        dists = np.absolute(dists)
        dists = np.sum(dists, axis=-1).T
